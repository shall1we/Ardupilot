...
bool LogReader::handle_log_format_msg(const struct log_Format &f)
{
	...
	if (streq(name, "PARM")) {
        ...
        msgparser[f.type] = new LR_MsgHandler_RFRF(formats[f.type], ekf2, ekf3, eahrs);
    } else if (streq(name, "RFRN")) {
        msgparser[f.type] = new LR_MsgHandler_RFRN(formats[f.type]);
    } else if (streq(name, "REV2")) {
        msgparser[f.type] = new LR_MsgHandler_REV2(formats[f.type], ekf2, ekf3, eahrs);
	} else if (streq(name, "RSO2")) {
        msgparser[f.type] = new LR_MsgHandler_RSO2(formats[f.type], ekf2, ekf3, eahrs);
	} else if (streq(name, "RWA2")) {
        msgparser[f.type] = new LR_MsgHandler_RWA2(formats[f.type], ekf2, ekf3, eahrs);
	} else if (streq(name, "REV3")) {
        msgparser[f.type] = new LR_MsgHandler_REV3(formats[f.type], ekf2, ekf3, eahrs);
	} else if (streq(name, "RSO3")) {
        msgparser[f.type] = new LR_MsgHandler_RSO3(formats[f.type], ekf2, ekf3, eahrs);
	} else if (streq(name, "RWA3")) {
        msgparser[f.type] = new LR_MsgHandler_RWA3(formats[f.type], ekf2, ekf3, eahrs);
	} else if (streq(name, "REY3")) {
        msgparser[f.type] = new LR_MsgHandler_REY3(formats[f.type], ekf2, ekf3, eahrs);
        ...
        msgparser[f.type] = new LR_MsgHandler_ROFH(formats[f.type], ekf2, ekf3, eahrs);
    } else if (streq(name, "REPH")) {
        msgparser[f.type] = new LR_MsgHandler_REPH(formats[f.type], ekf2, ekf3, eahrs);
	} else if (streq(name, "RSLL")) {
        msgparser[f.type] = new LR_MsgHandler_RSLL(formats[f.type], ekf2, ekf3, eahrs);
    } else if (streq(name, "REVH")) {
        msgparser[f.type] = new LR_MsgHandler_REVH(formats[f.type], ekf2, ekf3, eahrs);
    } else if (streq(name, "RWOH")) {
        msgparser[f.type] = new LR_MsgHandler_RWOH(formats[f.type], ekf2, ekf3, eahrs);
    } else if (streq(name, "RBOH")) {
        msgparser[f.type] = new LR_MsgHandler_RBOH(formats[f.type], ekf2, ekf3, eahrs);
	} else {
