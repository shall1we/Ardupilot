...
if PREV_ENABLE:get() == 0 then
   gcs:send_text(MAV_SEVERITY.NOTICE, string.format("Parameter switching disabled"))
   return
end
...
function update()
   ...
   if sw_pos ~= nil and sw_pos ~= prev_pos then
      ...
      gcs:send_text(MAV_SEVERITY.INFO, string.format("Switched %u parameters to %s", count, AuxSwitchPosNames[sw_pos + 1]))
      prev_pos = sw_pos
   end
end
